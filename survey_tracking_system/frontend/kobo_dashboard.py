...
def show_reports_page():
    ...
    # ==========================================
    # COMPREHENSIVE ANALYTICAL REPORT SECTION
    # ==========================================
    st.markdown("""
    <div class="section-divider" style="background: linear-gradient(135deg, #0066cc 0%, #3b82f6 100%); border-left: 4px solid #1e40af;">
        <h3 style="color: white;">📄 Comprehensive Analytical Report</h3>
        <p style="color: rgba(255,255,255,0.8);">Generate detailed PDF report with indicator analysis, averages, and recommendations</p>
    </div>
    """, unsafe_allow_html=True)
    
    st.markdown("""
    This comprehensive report includes:
    - **Executive Summary** with overall ICT readiness score
    - **Methodology** explaining the assessment framework
    - **Indicator Analysis** with calculated averages and data tables (no charts)
    - **Regional Analysis** with readiness scores by region
    - **National Analysis** with summary statistics
    - **Detailed Data Tables** showing response frequencies
    - **Conclusions** based on findings
    - **Recommendations** for improvement
    """)
    
    st.info("💡 **Tip:** TXT format generates faster (~5-10 seconds). PDF format may take 20-40 seconds for large datasets.")
    
    col1, col2 = st.columns([2, 1])
    
    with col1:
        # Optional: NSA logo path input
        logo_path = st.text_input(
            "NSA Logo Path (optional)",
            placeholder="e.g., assets/nsa_logo.png",
            help="Provide path to NSA logo image to include in report header (PDF only)"
        )
    ...
    if st.button("📄 Generate Comprehensive Analytical Report", type="primary"):
        try:
            ...
            if not submissions or len(submissions) == 0:
                st.error("❌ No submission data available to generate report. Please ensure data is loaded.")
                st.stop()
            
            # Use spinner instead of progress bar to avoid UI blocking
            with st.spinner(f'🔄 Generating report for {len(submissions)} submissions... This may take 20-40 seconds. Please wait...'):
                # Progress callback function (prints to terminal only)
                ...
                if logo_path and os.path.exists(logo_path):
                    ...
                    st.warning(f"Logo file not found at: {logo_path}. Report will be generated without logo.")
                
                generated_files = {}
                
                # Generate PDF if requested
                if report_format in ["PDF", "Both (PDF + TXT)"]:
                    try:
                        ...
                        with open(pdf_path, 'rb') as pdf_file:
                            generated_files['pdf'] = pdf_file.read()
                ...
                if report_format in ["TXT", "Both (PDF + TXT)"]:
                    try:
                        with tempfile.NamedTemporaryFile(delete=False, suffix='.txt') as tmp_file:
                            txt_output_path = tmp_file.name
                        
                        txt_path = generate_comprehensive_report(
                            submissions=submissions,
                            output_path=txt_output_path,
                            logo_path=None,  # Logo not applicable for TXT
                            progress_callback=update_progress,
                            format='txt'
                        )
                        
                        with open(txt_path, 'r', encoding='utf-8') as txt_file:
                            generated_files['txt'] = txt_file.read()
                        
                        try:
                            os.unlink(txt_output_path)
                        except:
                            pass
                        
                    except Exception as txt_error:
                        st.error(f"TXT generation failed: {str(txt_error)}")
                        import traceback
                        st.code(traceback.format_exc())
                        raise
            ...
            if 'pdf' in generated_files and 'txt' in generated_files:
                col_dl1, col_dl2 = st.columns(2)
                with col_dl1:
                    st.download_button(
                        label="📥 Download PDF Report",
                        data=generated_files['pdf'],
                        file_name=f"GBV_Comprehensive_Report_{timestamp}.pdf",
                        mime="application/pdf",
                        width='stretch'
                    )
                with col_dl2:
                    st.download_button(
                        ...
                        data=generated_files['txt'],
                        file_name=f"GBV_Comprehensive_Report_{timestamp}.txt",
                        mime="text/plain",
                        width='stretch'
                    )
            elif 'pdf' in generated_files:
                st.download_button(
                    label="📥 Download PDF Report",
                    data=generated_files['pdf'],
                    file_name=f"GBV_Comprehensive_Report_{timestamp}.pdf",
            ...
            st.info(f"""
            **Report Statistics:**
            - Total Submissions Analyzed: {len(submissions)}
            - Report Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
            - Format: {report_format}
            - Includes: Comprehensive regional/national analysis, indicator averages, strategic way forward
            """)
    ...
    # ==========================================
    KEY_INDICATORS = {
        "ICT Support": {"key": "grp3/q3_1_4", "label": "Availability of ICT Budget"},
    ...
    INSTITUTION_GROUPS = {
        ...
        "Ministry of Health Services": {"color": "#22c55e", "keywords": ["clinic", "hospital", "health", "district"]},
        ...
        "Magistrate": {"color": "#f59e0b", "keywords": ["court", "magistrate", "justice","facility"]}
    }
    ...
    # ---- CHART 1: Institution Groups per Region BY INDICATOR (with filter) ----
